...
@Slf4j
@Service
@MessageRouterMapping(action = {"fbackup", "ibackup"}, object = "mysql", type = "storage", status = "2",
        before = {BackupFailedRetryJobHandler.class}, after = {ProcessQueueJobHandler.class})
public class MysqlBackupFinishExceptionHandler implements MessageHandler {
    @Override
    public void handle(MqMessage message) throws Exception {
        log.info("这是MySQL备份存储异常handler:{}", message.toString());
    }
}
